...
import {
  ...
  User, Phone, Mail, LogOut, Crown, ArrowRight, type LucideIcon
} from 'lucide-react';
...
const statusIcons: Record<Booking['booking_status'], LucideIcon> = {
  pending: AlertCircle,
...
export default function UserDashboard() {
  ...
  useEffect(() => {
    async function fetchBookings() {
      if (!user) return;
      const data = await bookingApi.list({ userId: user.id });
      setBookings(data || []);
      setLoading(false);
    }
  ...
  async function handleCancelBooking(id: string) {
    await bookingApi.update(id, { booking_status: 'cancelled' });
  ...
  async function handleSaveProfile() {
    if (!user) return;
    setSaveLoading(true);
    await profileApi.update(user.id, { name: profileForm.name, phone: profileForm.phone });
