...
import { getBranchPopupSettings, type BranchPopupSettings } from '../utils/branchPopupSettings';
...
export default function BranchPopup({ open, onClose }: BranchPopupProps) {
  const [settings, setSettings] = useState<BranchPopupSettings | null | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    popupApi.active()
      .then(popup => {
        if (!active) return;
        if (!popup) {
          setSettings(getBranchPopupSettings());
          return;
        }
      ...
      .catch(() => {
        if (active) setSettings(getBranchPopupSettings());
      });
  ...
  const titleEnd = titleParts.slice(-1).join(' ');
      ...
      {open && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4">
          ...
          <motion.div
            ...
            className="relative z-10 max-h-[94vh] w-full max-w-md overflow-hidden rounded-2xl"
          ...
          >
            ...
            <button
              onClick={onClose}
              className="absolute top-4 right-4 w-8 h-8 rounded-full border border-brown/20 flex items-center justify-center text-stone hover:text-earth hover:border-brown/40 transition-all z-10 bg-white/80"
            >
              <X className="w-4 h-4" />
            </button>

            <div className="mobile-scrollbar-safe max-h-[calc(94vh-6px)] overflow-y-auto bg-cream-light p-5 text-center sm:p-8">
              ...
              {settings.image ? (
                <img src={settings.image} alt={settings.title} loading="lazy" className="mb-5 h-32 w-full rounded-2xl object-cover sm:h-36" />
              ) : (
                <div
              ...
              <h2 className="mb-2 font-playfair text-2xl font-bold leading-tight text-earth sm:text-3xl">
                {titleLead}<br />
                <span className="text-gradient-brown">{titleEnd}</span>
              </h2>
              ...
              <div className="mb-6 grid grid-cols-1 gap-3 min-[360px]:grid-cols-2">
              ...
              <div className="grid gap-3 min-[360px]:grid-cols-2">
                <a
                  ...
                  className="flex min-h-11 items-center justify-center gap-2 rounded-xl py-3 font-montserrat text-xs font-semibold text-white transition-all hover:opacity-90"
                ...
                <button
                  onClick={onClose}
                  className="btn-outline-gold min-h-11 justify-center py-3 text-xs"
