...
import {
  ...
  AlertCircle, Shield, Plus, Trash2, Edit3, ToggleLeft, ToggleRight
} from 'lucide-react';
import { supabase, Car as CarType, Booking } from '../lib/supabase';
...
export default function AdminPanel() {
  ...
  useEffect(() => {
    async function fetchData() {
      const [bookingRes, carRes, inquiryRes] = await Promise.all([
        supabase.from('bookings').select('*, cars(*), profiles(*)').order('created_at', { ascending: false }),
  ...
  async function updateBookingStatus(id: string, status: string) {
    ...
    setBookings(prev => prev.map(b => b.id === id ? { ...b, booking_status: status as any } : b));
  }

  async function toggleCarAvailability(id: string, current: boolean) {
    await supabase.from('cars').update({ availability: !current }).eq('id', id);
                    ...
                    <div className="p-5">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => toggleCarAvailability(car.id, car.availability)} className={`p-1.5 rounded-full border transition-colors ${car.availability ? 'bg-green-100 border-green-300 text-green-600' : 'bg-red-100 border-red-300 text-red-600'}`} title={car.availability ? 'Mark Unavailable' : 'Mark Available'}>
                            {car.availability ? <ToggleRight className="w-4 h-4" /> : <ToggleLeft className="w-4 h-4" />}
                          </button>
                          <button onClick={() => deleteCar(car.id)} className="p-1.5 rounded-full bg-red-100 border border-red-300 text-red-600 hover:bg-red-200 transition-colors">
